...
$packageFileName = Get-ChocolateyWebFile `
    ...
    -FileFullPath $(Join-Path $(GetTempDirChocoPackage) "$($packageName)Install.$fileType")`
    -Url $url `
    -Checksum $checksum `
    -ChecksumType 'sha512'
...
$sigFileName = Get-ChocolateyWebFile `
    -PackageName $packageName `
    -FileFullPath $(Join-Path $(GetTempDirChocoPackage) "$($packageName)Install.$fileType.asc")`
    -Url $urlSig `
